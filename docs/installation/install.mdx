...
###### Overview
 Choose your platform below to install Passionfruit. The same workspace syncs across the Home web app and the add-ins. To learn more about our applications, visit [App & Extensions](/docs/installation/apps).
...
<Tabs>
  <Tab title="Microsoft Office" icon="microsoft">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Refresh after new release">
        After a new release, you may need to refresh the add-in manifest to see the latest changes in Excel or Word. You can do this yourself; if you don't have the required permissions, ask your Microsoft 365 admin. Start by refreshing; only remove and re-add if the refresh doesn't show changes.
        1. Close all Office apps (Excel, Word, PowerPoint, Outlook)
        2. Reopen Excel/Word
        3. Go to `Home` in your top navigation. `My Add-ins` → `More Add-ins` → `Admin managed`, then click `Refresh` (top-right) 
        4. If changes still don't appear, remove Passionfruit from `My Add-ins`, then add it back from `My Add-ins` or the store
        <div className="my-4 rounded-lg overflow-hidden border">
        <iframe
          width="100%"
          height="360"
          src="https://www.loom.com/embed/eab1a0f72846490da1d8d69eabf113df?sid=docs"
          frameBorder="0"
          allowFullScreen
        />
        </div>
...
<div className=" rounded-lg bg-[#F8F7F4] overflow-hidden ">
  <video
    ...
    src="/videos/installation/install-google-chrome.mp4"
  ></video>
</div>
...
<div className=" rounded-lg bg-[#F8F7F4] overflow-hidden ">
  <video
    ...
    playsInline
    className="my-0  w-full rounded-md overflow-hidden"
    src="/videos/installation/Install-Microsoft Edge.mp4"
  ></video>
</div>
